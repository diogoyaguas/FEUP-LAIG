...
class XMLscene extends CGFscene {
   ...
   */
  initAxis() { this.axis = new CGFaxis(this, this.graph.axisLength); }
  ...
  initViews() {
    ...
    for (var id in views) {
      ...
      if (view.type == "perspective") {
        ...
        var near = view[0], far = view[1], left = view[2], rigth = view[3],
            top = view[4], bottom = view[5];
        this.views[id] = new CGFcameraOrtho(left, rigth, bottom, top, near, far,
                                            position, target, up)
      }
    }

    this.camera = this.views[this.graph.default];
    this.interface.setActiveCamera
    (this.camera);
  }
  ...
  */
  initAmbient() {
    this.setGlobalAmbientLight(
        ...
        this.graph.globalAmbient[2], this.graph.globalAmbient[3]);
    this.gl.clearColor(this.graph.background[0], this.graph.background[1],
  ...
  initLights() {
    ...
    for (var key in this.graph.lights) {
      ...
      if (this.graph.lights.hasOwnProperty(key)) {
        ...
        this.lights[i].setPosition(light[1][0], light[1][1], light[1][2],
                                   light[1][3]);
        this.lights[i].setAmbient(light[2][0], light[2][1], light[2][2],
                                  light[2][3]);
        this.lights[i].setDiffuse(light[3][0], light[3][1], light[3][2],
                                  light[3][3]);
        this.lights[i].setSpecular(light[4][0], light[4][1], light[4][2],
                                   light[4][3]);
